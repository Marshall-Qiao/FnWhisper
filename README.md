...
新机器也可以执行 `./scripts/bootstrap-machine.sh`，按 `Brewfile` 安装运行依赖、下载并校验 `large-v3-q5_0` 模型，然后构建和安装 App。已验证的公开部署基线见 [docs/DEPLOYMENT_MACHINE.md](docs/DEPLOYMENT_MACHINE.md)。
...
5. 输出层仅接受中文、英文、中英混合和标点，拒绝其他文字脚本。
...
实现取舍和权限边界见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
